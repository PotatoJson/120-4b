class LevelTutorial extends Phaser.Scene {
    constructor() {
        super("levelTutorialScene"); // Unique key for this level
    }
    ...
    create() {
        ...
        this.map = this.add.tilemap("levelTutorial", 16, 16, 150, 40); // Tutorial Map
        ...
        // Helper function to add text at specific TILE coordinates
        const addTutorialText = (tileX, tileY, text) => {
            // Calculate world coordinates by multiplying by tile size and layer scale
            const worldX = tileX * this.map.tileWidth * this.groundLayer.scaleX;
            const worldY = tileY * this.map.tileHeight * this.groundLayer.scaleX;
            
            this.add.text(worldX, worldY, text, tutorialTextStyle)
            .setOrigin(0.5, 1)// Origin at bottom-center
            .setDepth(100);; 
        };

        // --- Add Text to Circled Areas ---
        // NOTE: These are estimated tile coordinates. You may need to adjust them.

        // Text for circles from left to right:
        addTutorialText(5, 24, 'Use Arrow Keys\nto Move');
        addTutorialText(13.5, 28, 'Press [Z]\nto Jump');
        addTutorialText(19, 22, 'Press and hold [Z]\nto Jump higher');
        addTutorialText(31.5, 22, 'Press [C]\nto Dash');
        addTutorialText(44, 25, 'Hold [Down] to crouch');
        addTutorialText(54, 25, 'Hold [Down] and press [C]\nto crouch dash');
        addTutorialText(68, 28, 'Jump and Press [C]\nto air dash');
        addTutorialText(80, 15, 'Jump into a wall holding a direction\nto Wallslide and push [Z]\n to Wall Jump');
        addTutorialText(100, 7, 'Hold down + [Z] to \nfall through platforms');
        addTutorialText(110, 13, 'You can Pogo on\n spikes with [X] while in the air');
        addTutorialText(113, 24, 'These crystals refresh\nyour jump and dash!');
        addTutorialText(117, 32, 'These \'Diamonds\' \nare just for fun!');
        ...
        this.cameras.main.setBounds(0, 0, this.map.widthInPixels * this.groundLayer.scaleX, (this.map.heightInPixels * this.groundLayer.scaleY)-225); //

        // --- Player Creation ---
        // Determine player's starting position for this level (e.g., from Tiled object layer or hardcoded)
        this.playerStartX = 20;
        this.playerStartY = 800; // Assuming player starts on the ground layer
